(function($){
    ...
    $.fn.bobBanner = function(){
        //初始化轮播图html begins
        var $dot = $('<div class="dot"></div>');
        for (var i = 0; i < arguments[0].length; i++){
            $('.banner-container').append(
                $('<a></a>').attr('href', arguments[0][i].href).append(
                    $('<div></div>').addClass('banner').css({
                        'background': 'url(' + arguments[0][i].url + ') no-repeat'
                    })
                )
            );

            $dot.append($('<span></span>').attr('index', i));
        }

        $($('.banner-container .banner')[0]).addClass('active');
        $('.banner-container').append('<a href="javacript:;" class="button prev"></a>');
        $('.banner-container').append('<a href="javacript:;" class="button next"></a>');
        $('.banner-container').append($dot);
        $($('.banner-container .dot span')[0]).addClass('active');
        //初始化轮播图html ends
